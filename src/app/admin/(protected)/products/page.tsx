...
export default function AdminProductsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        const updateData: any = { title, price: parseFloat(price), category_id: categoryId };
        ...
        const { error: dbError } = await supabase.from('products').insert({
          title, price: parseFloat(price), category_id: categoryId, display_order: localProducts.length, image_url: imageUrl
        });
        if (dbError) throw dbError;
