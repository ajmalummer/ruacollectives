...
export function StoreProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const [{ data: cats }, { data: prods }, { data: signs }] = await Promise.all([
  ...
  return (
    <StoreContext.Provider value={{ categories, products, signIns, isAuthenticated, login, logout, fetchData, isLoading }}>
      {children}
